...
// DeserializeTravel creates a Travel from a map representation that has been
// unmarshalled from a text or binary format.
func DeserializeTravel(m map[string]interface{}, aliasMap map[string]string) (*Travel, error) {
	alias := ""
	aliasPrefix := ""
	...
	if typeValue, ok := m["type"]; !ok {
		...
		if typeName != "Travel" {
			return nil, fmt.Errorf("\"type\" property is not of %q type: %s", "Travel", typeName)
		}
		// Fall through, success in finding a proper Type
	} else if arrType, ok := typeValue.([]interface{}); ok {
		found := false
		for _, elemVal := range arrType {
			if typeString, ok := elemVal.(string); ok && strings.TrimPrefix(typeString, aliasPrefix) == "Travel" {
				found = true
				break
			}
		}
		if !found {
			return nil, fmt.Errorf("could not find a \"type\" property of value %q", "Travel")
		}
		// Fall through, success in finding a proper Type
	} else {
...
func (this Travel) GetUnknownProperties() map[string]interface{} {
	return this.unknown
}

// GetUpdated returns the "updated" property if it exists, and nil otherwise.
func (this Travel) GetUpdated() vocab.UpdatedPropertyInterface {
	return this.Updated
}

// GetUrl returns the "url" property if it exists, and nil otherwise.
func (this Travel) GetUrl() vocab.UrlPropertyInterface {
	return this.Url
}

// IsExtending returns true if the Travel type extends from the other type.
func (this Travel) IsExtending(other vocab.Type) bool {
	return TravelExtends(other)
}

// JSONLDContext returns the JSONLD URIs required in the context string for this
// type and the specific properties that are set. The value in the map is the
// alias used to import the type and its properties.
func (this Travel) JSONLDContext() map[string]string {
...
// LessThan computes if this Travel is lesser, with an arbitrary but stable
// determination.
func (this Travel) LessThan(o vocab.TravelInterface) bool {
...
func (this Travel) Serialize() (map[string]interface{}, error) {
	m := make(map[string]interface{})
	typeName := "Travel"
	if len(this.alias) > 0 {
		typeName = this.alias + ":" + "Travel"
	}
	m["type"] = typeName
