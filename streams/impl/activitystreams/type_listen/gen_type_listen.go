...
// DeserializeListen creates a Listen from a map representation that has been
// unmarshalled from a text or binary format.
func DeserializeListen(m map[string]interface{}, aliasMap map[string]string) (*Listen, error) {
	alias := ""
	aliasPrefix := ""
	...
	if typeValue, ok := m["type"]; !ok {
		...
		if typeName != "Listen" {
			return nil, fmt.Errorf("\"type\" property is not of %q type: %s", "Listen", typeName)
		}
		// Fall through, success in finding a proper Type
	} else if arrType, ok := typeValue.([]interface{}); ok {
		found := false
		for _, elemVal := range arrType {
			if typeString, ok := elemVal.(string); ok && strings.TrimPrefix(typeString, aliasPrefix) == "Listen" {
				found = true
				break
			}
		}
		if !found {
			return nil, fmt.Errorf("could not find a \"type\" property of value %q", "Listen")
		}
		// Fall through, success in finding a proper Type
	} else {
...
// ListenExtends returns true if the Listen type extends from the other type.
func ListenExtends(other vocab.Type) bool {
...
func (this Listen) GetUnknownProperties() map[string]interface{} {
	return this.unknown
}

// GetUpdated returns the "updated" property if it exists, and nil otherwise.
func (this Listen) GetUpdated() vocab.UpdatedPropertyInterface {
	return this.Updated
}

// GetUrl returns the "url" property if it exists, and nil otherwise.
func (this Listen) GetUrl() vocab.UrlPropertyInterface {
	return this.Url
}

// IsExtending returns true if the Listen type extends from the other type.
func (this Listen) IsExtending(other vocab.Type) bool {
	return ListenExtends(other)
}

// JSONLDContext returns the JSONLD URIs required in the context string for this
// type and the specific properties that are set. The value in the map is the
// alias used to import the type and its properties.
func (this Listen) JSONLDContext() map[string]string {
...
// LessThan computes if this Listen is lesser, with an arbitrary but stable
// determination.
func (this Listen) LessThan(o vocab.ListenInterface) bool {
...
func (this Listen) Serialize() (map[string]interface{}, error) {
	m := make(map[string]interface{})
	typeName := "Listen"
	if len(this.alias) > 0 {
		typeName = this.alias + ":" + "Listen"
	}
	m["type"] = typeName
