...
// DeserializeUpdate creates a Update from a map representation that has been
// unmarshalled from a text or binary format.
func DeserializeUpdate(m map[string]interface{}, aliasMap map[string]string) (*Update, error) {
	alias := ""
	aliasPrefix := ""
	...
	if typeValue, ok := m["type"]; !ok {
		...
		if typeName != "Update" {
			return nil, fmt.Errorf("\"type\" property is not of %q type: %s", "Update", typeName)
		}
		// Fall through, success in finding a proper Type
	} else if arrType, ok := typeValue.([]interface{}); ok {
		found := false
		for _, elemVal := range arrType {
			if typeString, ok := elemVal.(string); ok && strings.TrimPrefix(typeString, aliasPrefix) == "Update" {
				found = true
				break
			}
		}
		if !found {
			return nil, fmt.Errorf("could not find a \"type\" property of value %q", "Update")
		}
		// Fall through, success in finding a proper Type
	} else {
...
// UpdateExtends returns true if the Update type extends from the other type.
func UpdateExtends(other vocab.Type) bool {
...
func (this Update) GetUnknownProperties() map[string]interface{} {
	return this.unknown
}

// GetUpdated returns the "updated" property if it exists, and nil otherwise.
func (this Update) GetUpdated() vocab.UpdatedPropertyInterface {
	return this.Updated
}

// GetUrl returns the "url" property if it exists, and nil otherwise.
func (this Update) GetUrl() vocab.UrlPropertyInterface {
	return this.Url
}

// IsExtending returns true if the Update type extends from the other type.
func (this Update) IsExtending(other vocab.Type) bool {
	return UpdateExtends(other)
}

// JSONLDContext returns the JSONLD URIs required in the context string for this
// type and the specific properties that are set. The value in the map is the
// alias used to import the type and its properties.
func (this Update) JSONLDContext() map[string]string {
...
// LessThan computes if this Update is lesser, with an arbitrary but stable
// determination.
func (this Update) LessThan(o vocab.UpdateInterface) bool {
...
func (this Update) Serialize() (map[string]interface{}, error) {
	m := make(map[string]interface{})
	typeName := "Update"
	if len(this.alias) > 0 {
		typeName = this.alias + ":" + "Update"
	}
	m["type"] = typeName
