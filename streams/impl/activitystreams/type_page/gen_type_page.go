...
// DeserializePage creates a Page from a map representation that has been
// unmarshalled from a text or binary format.
func DeserializePage(m map[string]interface{}, aliasMap map[string]string) (*Page, error) {
	alias := ""
	aliasPrefix := ""
	...
	if typeValue, ok := m["type"]; !ok {
		...
		if typeName != "Page" {
			return nil, fmt.Errorf("\"type\" property is not of %q type: %s", "Page", typeName)
		}
		// Fall through, success in finding a proper Type
	} else if arrType, ok := typeValue.([]interface{}); ok {
		found := false
		for _, elemVal := range arrType {
			if typeString, ok := elemVal.(string); ok && strings.TrimPrefix(typeString, aliasPrefix) == "Page" {
				found = true
				break
			}
		}
		if !found {
			return nil, fmt.Errorf("could not find a \"type\" property of value %q", "Page")
		}
		// Fall through, success in finding a proper Type
	} else {
...
// PageExtends returns true if the Page type extends from the other type.
func PageExtends(other vocab.Type) bool {
...
func (this Page) GetUnknownProperties() map[string]interface{} {
	return this.unknown
}

// GetUpdated returns the "updated" property if it exists, and nil otherwise.
func (this Page) GetUpdated() vocab.UpdatedPropertyInterface {
	return this.Updated
}

// GetUrl returns the "url" property if it exists, and nil otherwise.
func (this Page) GetUrl() vocab.UrlPropertyInterface {
	return this.Url
}

// IsExtending returns true if the Page type extends from the other type.
func (this Page) IsExtending(other vocab.Type) bool {
	return PageExtends(other)
}

// JSONLDContext returns the JSONLD URIs required in the context string for this
// type and the specific properties that are set. The value in the map is the
// alias used to import the type and its properties.
func (this Page) JSONLDContext() map[string]string {
...
// determination.
func (this Page) LessThan(o vocab.PageInterface) bool {
...
func (this Page) Serialize() (map[string]interface{}, error) {
	m := make(map[string]interface{})
	typeName := "Page"
	if len(this.alias) > 0 {
		typeName = this.alias + ":" + "Page"
	}
	m["type"] = typeName
