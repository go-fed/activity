...
// DeserializeOffer creates a Offer from a map representation that has been
// unmarshalled from a text or binary format.
func DeserializeOffer(m map[string]interface{}, aliasMap map[string]string) (*Offer, error) {
	alias := ""
	aliasPrefix := ""
	...
	if typeValue, ok := m["type"]; !ok {
		...
		if typeName != "Offer" {
			return nil, fmt.Errorf("\"type\" property is not of %q type: %s", "Offer", typeName)
		}
		// Fall through, success in finding a proper Type
	} else if arrType, ok := typeValue.([]interface{}); ok {
		found := false
		for _, elemVal := range arrType {
			if typeString, ok := elemVal.(string); ok && strings.TrimPrefix(typeString, aliasPrefix) == "Offer" {
				found = true
				break
			}
		}
		if !found {
			return nil, fmt.Errorf("could not find a \"type\" property of value %q", "Offer")
		}
		// Fall through, success in finding a proper Type
	} else {
...
// OfferExtends returns true if the Offer type extends from the other type.
func OfferExtends(other vocab.Type) bool {
...
func (this Offer) GetUnknownProperties() map[string]interface{} {
	return this.unknown
}

// GetUpdated returns the "updated" property if it exists, and nil otherwise.
func (this Offer) GetUpdated() vocab.UpdatedPropertyInterface {
	return this.Updated
}

// GetUrl returns the "url" property if it exists, and nil otherwise.
func (this Offer) GetUrl() vocab.UrlPropertyInterface {
	return this.Url
}

// IsExtending returns true if the Offer type extends from the other type.
func (this Offer) IsExtending(other vocab.Type) bool {
	return OfferExtends(other)
}

// JSONLDContext returns the JSONLD URIs required in the context string for this
// type and the specific properties that are set. The value in the map is the
// alias used to import the type and its properties.
func (this Offer) JSONLDContext() map[string]string {
...
// determination.
func (this Offer) LessThan(o vocab.OfferInterface) bool {
...
func (this Offer) Serialize() (map[string]interface{}, error) {
	m := make(map[string]interface{})
	typeName := "Offer"
	if len(this.alias) > 0 {
		typeName = this.alias + ":" + "Offer"
	}
	m["type"] = typeName
