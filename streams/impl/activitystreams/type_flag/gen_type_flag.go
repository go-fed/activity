...
// DeserializeFlag creates a Flag from a map representation that has been
// unmarshalled from a text or binary format.
func DeserializeFlag(m map[string]interface{}, aliasMap map[string]string) (*Flag, error) {
	alias := ""
	aliasPrefix := ""
	...
	if typeValue, ok := m["type"]; !ok {
		...
		if typeName != "Flag" {
			return nil, fmt.Errorf("\"type\" property is not of %q type: %s", "Flag", typeName)
		}
		// Fall through, success in finding a proper Type
	} else if arrType, ok := typeValue.([]interface{}); ok {
		found := false
		for _, elemVal := range arrType {
			if typeString, ok := elemVal.(string); ok && strings.TrimPrefix(typeString, aliasPrefix) == "Flag" {
				found = true
				break
			}
		}
		if !found {
			return nil, fmt.Errorf("could not find a \"type\" property of value %q", "Flag")
		}
		// Fall through, success in finding a proper Type
	} else {
...
// FlagExtends returns true if the Flag type extends from the other type.
func FlagExtends(other vocab.Type) bool {
...
func (this Flag) GetUnknownProperties() map[string]interface{} {
	return this.unknown
}

// GetUpdated returns the "updated" property if it exists, and nil otherwise.
func (this Flag) GetUpdated() vocab.UpdatedPropertyInterface {
	return this.Updated
}

// GetUrl returns the "url" property if it exists, and nil otherwise.
func (this Flag) GetUrl() vocab.UrlPropertyInterface {
	return this.Url
}

// IsExtending returns true if the Flag type extends from the other type.
func (this Flag) IsExtending(other vocab.Type) bool {
	return FlagExtends(other)
}

// JSONLDContext returns the JSONLD URIs required in the context string for this
// type and the specific properties that are set. The value in the map is the
// alias used to import the type and its properties.
func (this Flag) JSONLDContext() map[string]string {
...
// determination.
func (this Flag) LessThan(o vocab.FlagInterface) bool {
...
func (this Flag) Serialize() (map[string]interface{}, error) {
	m := make(map[string]interface{})
	typeName := "Flag"
	if len(this.alias) > 0 {
		typeName = this.alias + ":" + "Flag"
	}
	m["type"] = typeName
